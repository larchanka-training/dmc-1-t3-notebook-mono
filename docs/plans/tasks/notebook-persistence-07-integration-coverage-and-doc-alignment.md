...
- покрыть anonymous access, owner-only access, valid create defaults и validation failures
...
- [ ] Integration tests покрывают anonymous `401`, owner-only `404`, create/list/get/patch/delete flows и invalid notebook payload paths
- [ ] Notebook tests используют рабочий authenticated fixture или эквивалентный contract-faithful session issuance path
- [ ] Документация согласована по notebook access semantics, минимум для `404` owner-only behavior и persistence fields
