...
- [ ] `ui/docs/runtime_architecture.md` описывает live worker session как реализованную current model без ссылки на replay как основной session mechanism
- [ ] `ui/docs/adr/ADR-003-runtime-execution-model.md` остаётся согласованным с фактическим runtime behavior после migration
- [ ] `docs/plans/06-live-worker-session-transition-plan.md` обновлён как закрытый или актуализированный plan artifact с явным итогом migration
- [ ] Если `docs/qa_plan.md` или related task specs ссылаются на устаревшие replay assumptions, они синхронизированы с новой semantics
- [ ] В документации явно сохранены текущие boundaries: client-side runtime, frontend-side orchestrator, transient outputs, coarse-grained stop/timeout
