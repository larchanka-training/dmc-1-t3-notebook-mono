...
- для local/dev smoke: backend container image должен содержать `node`, потому что deterministic JavaScript syntax validation использует `node --check`
...
Для local/dev smoke дополнительно проверьте:

8. `POST /api/v1/auth/request-otp`
...
- `ready=true` не доказывает, что deterministic syntax validation сможет выполниться
- для текущей backend реализации в runtime image также нужен `node`
- если `node` отсутствует, backend может ошибочно вернуть `AI_CODE_SYNTAX_INVALID`, даже когда Bedrock уже вернул валидный JavaScript
...
Для local/dev smoke auth flow тоже должен быть operational:

- `POST /api/v1/auth/request-otp` должен возвращать `200`
- в development-safe path должен возвращаться `dev_otp`
- backend не должен пытаться отправлять OTP через `SES`, если включён dev OTP flow

Без working auth/session flow Bedrock smoke для notebook AI не является полноценным end-to-end smoke.
...
| health выглядит нормально, но AI route даёт `AI_CODE_SYNTAX_INVALID` на валидном code response | в backend runtime image отсутствует `node` для syntax validation |
| `POST /api/v1/auth/request-otp` падает в local/dev | backend пытается использовать `SES` вместо development-safe OTP path |
