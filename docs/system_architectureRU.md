...
1. Пользователь пишет AI-запрос в `text` block.
2. Для корректировки существующего `code` block в Version 1 frontend может сначала преобразовать этот `code` block в `text` block, который сохраняет предыдущий код и принимает инструкцию на изменение.
...
6. Frontend вставляет сгенерированный код в следующий пустой `code` block после исходного `text` block или создает там новый `code` block.
7. Пользователь просматривает и редактирует сгенерированный код.
8. Предыдущий код может оставаться видимым в исходном `text` block для документации и сравнения, а новый результат остается обычным редактируемым notebook-блоком.
